...
It then hands the merge to you as a keyed decision - `needs-decision [key=merge]:` carrying that URL, `direct-PR` once the PR is open and `no-mistakes` once CI is green - and stops, because the merge needs your authority to move and is not a wait that clears on its own.
Answer it with `fm-send --resolve-key` like any other decision once the PR reaches a terminal state, and the worker reports the delivery, so a ship task's delivery `done:` means merged or closed without merging, never a pushed branch, an open PR, or green CI.
