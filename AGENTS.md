...
It then reports that URL on a `working:` line - `direct-PR` once the PR is open, `no-mistakes` once CI is green - and holds until the PR reaches a terminal state, so the `done:` that reports a ship task's delivery means merged or closed without merging, never a pushed branch, an open PR, or green CI.
A `no-mistakes` worker still hands its implementation back with an earlier `done:` so you can trigger validation, exactly as the Validate section above describes; that one reports the implementation, not the delivery, and its brief says so where it tells the worker to write it.
Neither that `working:` line nor the merge wait the worker declares after it is captain-relevant, and the armed poll fires only on a merge, so nothing announces the open PR as a delivery: it reaches you as the ordinary notification for a crew that has gone quiet, then on that declared wait's bounded recheck and on section 8's fleet review - the recorded `pr=` and the URL in the declared wait are what tell you a quiet crew is holding for your merge authority rather than wedged.
