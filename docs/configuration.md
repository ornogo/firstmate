...
A standalone-clone home cannot receive a primary-local commit through that no-fetch sync, so it receives the rule through the operator-run `bin/fm-update.sh` origin refresh instead.
