...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, an operator-run update, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `claude opus` keeps a secondmate pinned to Opus even if the primary's own default model later changes.
...
`bin/fm-update.sh` first updates the remote code root from its own origin, then runs that guarded home sync.
